...
%%     Collada export.
%%
%%  Copyright (c) 2009 David Parfitt
%%
...
segment_by_material(#e3d_mesh{fs=Fs}) ->
    ...
    {Segs,_} = foldl(fun (#e3d_face{mat=Mats}=Face, {Tree0,Index}) ->
		  Tree1= case gb_trees:lookup(Mats, Tree0) of
			     ...
			     none -> gb_trees:insert(Mats, [{Index,Face}], Tree0)
			 end,	    		    
		  {Tree1,Index+1}
...
use_material(Name, MatDefs,#c_exp{matl_defs=ExpMatlDefs}=ExportState) ->
    IsElem = gb_trees:is_defined(Name, ExpMatlDefs),
    NewExpState = 
	if 
	    IsElem =:= false -> define_material(Name, MatDefs, ExportState);    
	    true -> ExportState		    
	end,
    NewExpState.
...
define_material(Name, MatDefs, #c_exp{matl_defs=ExpMatlDefs}=ExportState) ->
    ...
    Children = 
	[Emission, Ambient, Specular, Shininess, ColladaMatl1#c_matl.diffnode],
    Phong = {phong,[],Children},        
    Technique = {technique,[{sid,"wings3d"}],[Phong]},        
    ProfileChildren = 
	[ColladaMatl1#c_matl.surface,ColladaMatl1#c_matl.sampler2d,Technique],
...
make_mesh_source_pos(Name, Mesh) ->
    ...
    TechCommon = {technique_common,
		  [make_source_accessor(Name ++ "-Pos-array",length(Verts))]},
...
make_mesh_source_normals(Name, Mesh) ->
    ...
    TechCommon = {technique_common,
		  [make_source_accessor(Name ++ "-Normal-array",length(Normals))]},
...
make_mesh_uvs(Name, #e3d_mesh{tx=Tx}) ->
    ...
    TechCommon = {technique_common,
		  [make_uvsource_accessor(Name ++ "-UV-array",length(UVs))]},
    {{source,[{id,Name ++ "-UV"}],
      [FloatArray,TechCommon]},trunc(length(UVs)/2)-1}.
...
make_source_accessor(Source, TotalVertCount) ->
    FaceCount = trunc(TotalVertCount / 3),
